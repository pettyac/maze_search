...
class MazeProblem(Problem):
    ...
    def heuristic(self, state):
        ...
        # Use Euclidean distance:  \/(q1 - p1)^2 + (q2 - p2)^2
        for s in self.goal_states:       
            distance = (((s[0] - state[0]) ** 2) + 
                        ((s[1] - state[1]) ** 2)) ** 0.5
        return distance
...
class UCSProblem(MazeProblem):
    def __init__(self, maze=None, initial_state=None, goal_states=None):
        super(UCSProblem, self).__init__(maze, initial_state, goal_states)

class GBFSProblem(MazeProblem):

    def __init__(self, maze=None, initial_state=None, goal_states=None):
        super(GBFSProblem, self).__init__(maze, initial_state, goal_states)
...
class AStarProblem(MazeProblem):
    ...
    def __init__(self, maze=None, initial_state=None, goal_states=None):
        super(AStarProblem, self).__init__(maze, initial_state, goal_states)

    def f_cost(self, state, path_cost):
        return self.heuristic(state) + path_cost 
